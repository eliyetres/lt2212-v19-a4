...
parser.add_argument("sourcefile", type=str, default="UN-english", help="File used as source language.")
parser.add_argument("targetfile", type=str, default="UN-french", help="File used as target language..")
...
print("Loading data from {} and {}.".format(args.targetfile, args.sourcefile))
...
print("Sample length: ", len(init_sample), "batch size: ", b)
# Initiate network weights from sample 
X, Y = gen_tri_vec_split(init_sample, vectors, target_indices, target_len)   
# Train translation model
trigram_target_model.start(X, Y, layer_size, len(Y[0]))
for i in range(0, len(target_trigrams), b):
    X, Y = gen_tri_vec_split(target_trigrams[i:i+b], vectors, target_indices, target_len)        
...
X_list, Y_list = generate_translation_vectors(target_train[i_slice:i_slice+l], source_train[i_slice:i_slice+l], vectors, source_indices, l)
translation_model.start(X_list, Y_list, layer_size, len(Y_list[0]))  
# Train translation model
for i in range(0, len(target_train), b):  
    X_list, Y_list = generate_translation_vectors(target_train[i:i+b], source_train[i:i+b], vectors, source_indices, l)
    translation_model.train(X_list, Y_list, epochs)  

test_new(target_test, source_test, target_vocab, source_vocab, vectors, target_indices, source_indices, target_len, source_len, trigram_target_model, translation_model, p, args.top_predicted)